...
function base.highlights()
  ...
  return {
    Bold = { bold = true },
    Boolean = { fg = c.frost.arctic_water }, --  a boolean constant: TRUE, false
    Character = { fg = c.aurora.green }, --  a character constant: 'c', '\n'
    Conditional = { fg = c.frost.arctic_water }, --  if, then, else, endif, switch, etc.
    ...
    Define = { fg = c.frost.arctic_water }, --   preprocessor #define
    ...
    Exception = { fg = c.frost.arctic_water }, --  try, catch, throw
    ...
    Include = { fg = c.frost.arctic_water }, --  preprocessor #include
    Italic = { italic = true },
    Keyword = vim.tbl_extend("force", { fg = c.frost.arctic_water }, options.styles.keywords), --  any other keyword
    Label = { fg = c.frost.arctic_water }, --    case, default, etc.
    Macro = { link = "Define" }, --    same as Define
    Number = { fg = c.aurora.purple }, --   a number constant: 234, 0xff
    Operator = { fg = c.frost.arctic_water }, -- "sizeof", "+", "*", etc.
    PreCondit = { link = "PreProc" }, --  preprocessor #if, #else, #endif, etc.
    PreProc = { fg = c.frost.arctic_water }, -- (preferred) generic Preprocessor
    Repeat = { fg = c.frost.arctic_water }, --   for, do, while, etc.
    ...
    Statement = { fg = c.frost.arctic_water }, -- (preferred) any statement
    StorageClass = { fg = c.frost.arctic_water }, -- static, register, volatile, etc.
    String = { fg = c.aurora.green }, --   a string constant: "this is a string"
    Structure = { fg = c.frost.arctic_water }, --  struct, union, enum, etc.
    ...
    Type = { fg = c.frost.arctic_water }, -- (preferred) int, long, char, etc.
    Typedef = { fg = c.frost.arctic_water }, --  A typedef
    ...
    debugPC = { bg = utils.darken(c.frost.arctic_water, 0.3) }, -- used for highlighting the current line in terminal-debug
    debugBreakpoint = { bg = utils.darken(c.frost.arctic_ocean, 0.1), fg = c.frost.arctic_water }, -- used for breakpoint colors in terminal-debug
  }
end
